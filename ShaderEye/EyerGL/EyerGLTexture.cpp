...
namespace Eyer
{
    ...
    int EyerGLTexture::SetDataRGBChannel(unsigned char * data,int width,int height)
    {
#ifdef QT_EYER_PLAYER
        ctx->glBindTexture(GL_TEXTURE_2D, textureId);
        ctx->glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
        ctx->glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
        ctx->glBindTexture(GL_TEXTURE_2D, 0);
#else
        glBindTexture(GL_TEXTURE_2D, textureId);
        glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
